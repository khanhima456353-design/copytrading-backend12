...
type AccountSummary = { available: number; locked: number; unrealizedPnl?: number; equity?: number; holdings: AssetHolding[] };
type UserOrder = {
...
export default function Trading() {
  ...
  const refreshAccountData = useCallback(async () => {
    try {
      ...
      if (ordersRes?.data) {
        ...
        setLockedUSDT(orders
          .filter((o: UserOrder) => o.status === 'open' && o.side === 'buy')
          .reduce((sum: number, order: UserOrder) => sum + order.price * order.amount, 0));
      }
  ...
  const reservedUSDT = lockedUSDT;
  const availableBalanceRaw = Number.isFinite(accountSummary.available) ? accountSummary.available : 0;
  const freeToTrade = Math.max(0, availableBalanceRaw - reservedUSDT);
  const availableBalance = freeToTrade;
  const accountEquity = typeof accountSummary.equity === "number" ? accountSummary.equity : NaN;
  const accountUnrealizedPnl = typeof accountSummary.unrealizedPnl === "number" ? accountSummary.unrealizedPnl : 0;
  const totalEquity = Number.isFinite(accountEquity)
    ? accountEquity
    : availableBalanceRaw + accountUnrealizedPnl;
  ...
  const portfolioTotal = accountSummary.holdings.reduce((s, h) => s + h.value, availableBalanceRaw);
    ...
    : <>
      ...
      {serverPositions.map((p, idx) => (
        <div key={p.id || p._id || String(idx)} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr 1fr 100px", padding: "6px 14px", fontSize: 11, fontFamily: "monospace", borderBottom: `1px solid ${COLORS.border}` }}>
          ...
          <span style={{ color: COLORS.text }}>{(p.markPrice ?? p.entryPrice)?.toFixed(2)}</span>
          <span style={{ color: COLORS.text }}>{(p.quantity ?? p.size ?? 0).toFixed(4)}</span>
          <span style={{ color: (p.unrealizedPnl ?? 0) >= 0 ? COLORS.green : COLORS.red }}>{(p.unrealizedPnl ?? 0) >= 0 ? "+" : ""}{(p.unrealizedPnl ?? 0).toFixed(2)}</span>
