...
const ensureWallet = async (userId, type) => {
  ...
  let wallet = await Wallet.findOneAndUpdate(
    ...
    { $setOnInsert: { userId, type: walletType, availableBalance: 0, lockedBalance: 0, borrowedBalance: 0, unrealizedPnl: 0, equity: 0 } },
...
const lockFunds = async ({ userId, amount, walletType = "spot", reason, referenceType, referenceId }) => {
  if (!amount || amount <= 0) throw new Error("Lock amount must be positive");
  ...
  if (!wallet) {
    ...
    if (existing.isFrozen)  throw new Error("Wallet is frozen");
...
const unlockFunds = async ({ userId, amount, walletType = "spot", reason, referenceType, referenceId }) => {
  if (!amount || amount <= 0) throw new Error("Unlock amount must be positive");
...
const adjustBalances = async ({ userId, walletType = "spot", available = 0, locked = 0, borrowed = 0, adminId, description }) => {
  ...
  if (available < 0) filter.availableBalance = { $gte: -available };
  if (locked    < 0) filter.lockedBalance    = { $gte: -locked };
  if (borrowed  < 0) filter.borrowedBalance  = { $gte: -borrowed };

  const wallet = await Wallet.findOneAndUpdate(
    filter,
    { $inc: { availableBalance: available, lockedBalance: locked, borrowedBalance: borrowed } },
    { new: true }
  );
