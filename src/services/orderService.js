...
const validateOrder = async ({ userId, walletType, pair, side, type, price, quantity, stopPrice, feeRate }) => {
  ...
  if (!["spot", "cross", "isolated"].includes(walletType)) throw new Error("Unsupported wallet type");
  if (!["buy", "sell"].includes(side))                     throw new Error("Unsupported order side");
  if (!["limit", "market", "stop-loss"].includes(type))    throw new Error("Unsupported order type");
  if (type === "limit"     && !isValidPrice(price)) throw new Error("Limit orders require a valid price");
  if (type === "stop-loss" && !isValidPrice(stopPrice)) throw new Error("Stop-loss orders require a valid stop price");
  if (type === "market") getOrderMarketPrice(pair);

  const wallet = await WalletService.getWallet(
    userId,
    walletType === "isolated" ? "cross" : walletType
  );
  ...
  if (effectivePrice === null) throw new Error("Order requires a valid price");
...
const placeOrder = async ({ userId, walletType, pair, side, type, price, quantity, stopPrice, feeRate = 0 }) => {
  ...
  if (!isValidPrice(orderPrice)) throw new Error("Order requires a valid price");
...
const executeMarketOrder = async (order, _wallet) => {
  ...
  if (order.side === "buy") {
    ...
    await Wallet.findOneAndUpdate(
      { userId: order.userId, type: order.walletType, lockedBalance: { $gte: amount } },
    ...
    const Wallet = require("../../models/Wallet");
    await Wallet.findOneAndUpdate(
      { userId: order.userId, type: order.walletType },
...
const cancelOrder = async ({ orderId, userId }) => {
  ...
  const order = await Order.findOneAndUpdate(
    { _id: orderId, userId, status: "open" },
