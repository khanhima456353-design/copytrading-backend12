...
const getHolding = async (userId, asset) => {
  return Holding.findOneAndUpdate(
    { userId, asset },
    { $setOnInsert: { userId, asset, quantity: 0, lockedQuantity: 0, averagePrice: 0 } },
...
const addHolding = async ({ userId, asset, quantity, price }) => {
  if (!quantity || quantity <= 0) throw new Error("Add quantity must be positive");
  if (!price    || price    <= 0) throw new Error("Add price must be positive");
  ...
  const updated = await Holding.findOneAndUpdate(
    { userId, asset, quantity: prevQty },            // optimistic lock on qty
    {
...
const removeHolding = async ({ userId, asset, quantity }) => {
  if (!quantity || quantity <= 0) throw new Error("Remove quantity must be positive");
...
const lockHolding = async ({ userId, asset, quantity }) => {
  if (!quantity || quantity <= 0) throw new Error("Lock quantity must be positive");
...
const unlockHolding = async ({ userId, asset, quantity }) => {
  if (!quantity || quantity <= 0) throw new Error("Unlock quantity must be positive");
