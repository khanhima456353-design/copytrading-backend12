...
function withPairLock(pair, fn) {
  ...
  const next = previous.then(fn).catch((err) => console.error(`[pairLock:${pair}] error:`, err.message));
...
app.get('/api/account/summary', authMiddleware, async (req, res) => {
  try {
    ...
    const balance = balData?.USDT?.available || user.balance || 0;
    ...
    const filledBuyOrders = await Order.find({ userId, side: 'buy', status: 'filled' });
    const btcFromOrders = filledBuyOrders.filter(o=>o.pair&&o.pair.startsWith('BTC')).reduce((s,o)=>s+(o.amount||0),0);
    ...
    res.json({
      ...
      locked,
      holdings:  (()=>{
        const btcFromPos = positions.filter(p=>p.pair&&p.pair.startsWith('BTC')).reduce((s,p)=>s+Math.abs(p.size||0),0);
        const totalBTC = btcFromPos + btcFromOrders;
        const mp = cachedMarketPrices['BTC/USDT'] || 0;
        const base = positions.map((p) => { const mp2 = getEffectivePrice(userId, p.pair, global.cachedMarketPrices?.[p.pair] || p.entryPrice || 0); return { asset: p.pair.split('/')[0], amount: Math.abs(p.size), value: mp2 * Math.abs(p.size) }; });
        if (totalBTC > 0 && !base.find(h=>h.asset==='BTC')) base.push({ asset: 'BTC', amount: totalBTC, value: mp * totalBTC });
        return base;
      })(),
      totalBalance:    balance + (user.frozenBalance || 0),
      totalEquity:     totalEquity,
      usedMargin,
      freeMargin:      Math.max(0, totalEquity - usedMargin),
      dailyPnL:        0,
      winRate:         0,
...
app.post('/api/account/recalculate-balances', authMiddleware, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    if (!user)   return res.status(404).json({ message: 'User not found' });
    const existingBal = await balanceService.getBalances(userId);
    const hasBalance = existingBal?.USDT?.total > 0;
    const result = hasBalance ? existingBal : await balanceService.recalculateBalancesFromHistory(userId, user.balance || 0);
    const balances = await balanceService.getBalances(userId);
    return res.json({ message: 'Balances recalculated from trade history', calculated: result, current: { USDT: balances.USDT, BTC: balances.BTC } });
  } catch (err) {
...
const cachedValid   = isValidPriceValue(cached);
    ...
    if (type === 'market') {
      if (!cachedValid) {
        ...
        if (side === 'buy') { try { await balanceService.unlockBalance(userId, 'USDT', cost); } catch {} }
      ...
      o.status = 'filled'; o.filled = Number(amount); await o.save();
