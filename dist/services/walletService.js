...
async function recordTransaction(session, { userId, type, amount, balanceBefore, balanceAfter, lockedBefore, lockedAfter, reference, description }) {
    const options = session ? { session } : undefined;
    let ref = reference ?? null;
    if (ref !== null && typeof ref === "string") {
        try {
            ref = new mongoose.Types.ObjectId(ref);
        }
        catch {
            ref = null;
        }
...
async function creditBalance(userId, amount, description, type) {
    ...
    const before = await Wallet.findOneAndUpdate({ userId }, { $inc: { availableBalance: amount } }, { returnDocument: 'before' });
    if (!before) {
        throw new Error("Wallet not found");
    }
    const wallet = await Wallet.findOne({ userId });
...
async function lockBalance(userId, amount, orderId) {
    assertPositiveAmount(amount);
    const before = await Wallet.findOneAndUpdate({ userId, availableBalance: { $gte: amount } }, { $inc: { availableBalance: -amount, lockedBalance: amount } }, { returnDocument: 'before' });
    if (!before) {
...
async function unlockBalance(userId, amount, pnlAmount, orderId) {
    ...
    const before = await Wallet.findOneAndUpdate(filter, { $inc: { lockedBalance: -amount, availableBalance: amount + pnlAmount } }, { returnDocument: 'before' });
    if (!before) {
        const existing = await Wallet.findOne({ userId });
        if (!existing) {
            throw new Error("Wallet not found");
...
async function debitBalance(userId, amount, description) {
    assertPositiveAmount(amount);
    const before = await Wallet.findOneAndUpdate({ userId, availableBalance: { $gte: amount } }, { $inc: { availableBalance: -amount } }, { returnDocument: 'before' });
    if (!before) {
        const existing = await Wallet.findOne({ userId });
        if (!existing) {
            throw new Error("Wallet not found");
