...
type AccountSummary = { available: number; locked: number; unrealizedPnl?: number; holdings: AssetHolding[] };
type UserOrder = {
...
export default function Trading() {
  ...
  const closePosition = async (position: { pair: string; id?: string; _id?: string; markPrice?: number; entryPrice?: number; }) => {
    try {
      ...
      const response = await fetch('/api/pro/positions/isolated/close', {
        method: 'POST',
  ...
  const refreshAccountData = useCallback(async () => {
    try {
      ...
      if (positionsRes?.data) setServerPositions((positionsRes.data || []).filter((p: any) => p.status === "open"));
      if (ordersRes?.data) {
  ...
  const portfolioTotal = accountSummary.holdings.reduce((s, h) => s + h.value, availableBalance);
  ...
  const modeBalanceSummary = useMemo(() => {
    ...
    if (activeTab === "cross") return [
      ...
      { label: "Cross Used Margin", value: `$${lockedUSDT.toFixed(2)}` },
          ...
          <div onClick={e => e.stopPropagation()} style={{ width: 520, background: COLORS.bgPanel, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: "hidden", maxHeight: "80vh", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ overflow: "auto", flex: 1 }}>
              <div style={{ padding: "10px 20px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[{ label: "Available", value: `$${availableBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}` }, { label: "Locked", value: `$${lockedUSDT.toLocaleString(undefined, { minimumFractionDigits: 2 })}` }, { label: "Realized P&L", value: "$0.00" }, { label: "Rewards earned", value: "$0.00 USD" }].map(m => (
